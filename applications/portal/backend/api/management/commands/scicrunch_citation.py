...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        citation_metrics = self.fetch_scicrunch_citation_metric(antibodies_ids, api_key)
        if citation_metrics:
    ...
    def fetch_scicrunch_citation_metric(self, antibody_ids, api_key):
        citation_metrics = []
        ...
        for ab_id in antibody_ids:
            ...
            # link_for_api = f"https://api.scicrunch.io/elastic/v1/RIN_Tool_pr/_search?q={abid}&key={API_KEY}"
            link_for_api = f"https://scicrunch.org/api/1/elastic/RIN_Mentions_pr/data/_search?key={api_key}"
            ...
            if response.status_code == 200:
                ...
                self.stdout.write(self.style.ERROR(f"Failed to fetch citation metrics"))
