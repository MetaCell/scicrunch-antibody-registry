...
export const faqsInfo = [
  ...
  },
  {
    ...
    answer: <>No, the Antibody Registry is only for antibodies and kits that contain antibodies.
      Other proteins and dyes (e.g. Streptavidin, DAPI) do not need to be registered.
      There are legacy records in the registry that are not antibodies. They are tagged with a &apos;do not use this RRID&apos; message when found.<br />
  ...
  },
  {
    ...
    answer: <>Every vendor has their own way of formatting their catalog numbers.
      ...
      Many vendors append &apos;sizing&apos; information to the end of their catalog numbers (-100, S, L, 50uL).
      We try to provide only one RRID for a product - regardless of the size.
      Therefore, when registering or searching for products, do not include sizing information in the catalog number, if it can be easily differentiated.
      We will add known sizes in the alternative catalog number field when we know about them.<br />
      Vendor URLs should point to a page that contains information about the antibody being registered.
      If a URL with information is not available, you may be asked to provide some documentation like a technical data sheet.
    </>
  },
  {
    question: "What happens if I add duplicate submission?",
    answer: <>An antibody that is already registered does not need to be registered again.
      Sometimes a user submits a duplicate that has recently been registered and does not yet appear in the public record, this will result in a system message that states that it is a duplicate.
  ...
  },
  {
    ...
    answer: <>In 2022 we have opened up previously curator only fields that should help all users understand when each record was entered into the Antibody Registry and when it was last updated. In many cases there are several versions of an RRID record, available to users with the 2022 update.<br />
      Each RRID has a version that is the core record and potentially other auxiliary records. Commonly records that come from vendors that are known to be the same record (Human Protein Atlas antibodies available via Sigma and Atlas Antibodies), there are consolidated and rejected that were found to be duplicate (Cell Signaling small vs large vials of the same product), there are records that appear as duplicates due to bulk actions (catalog that was received from Chemicon vs Millipore, which were consolidated under one company name).<br />
      In all of these cases the resolver points to the core record (https://antibodyregistry.org/AB_#####) and will contain additional information about versions of that record if they exist.
    </>
  },
  {
  ...
  },
  {
    ...
    answer: <>
      The major groups that the Antibody Registry brings together include antibody companies (~300 are registered & see list of partners on the About page),
      researchers (&gt;10,000 registered users), and publishers (&gt;2000 <a href="https://rrids.org/journals">journals</a> contain at least one paper with at least one antibody RRID).
      However the problem of identifying antibodies in the scientific literature spans most of biomedicine and some of chemistry,
      thus nearly all biomedical researchers should be stakeholders and advocate for better antibody reagent identification.
    </>
  },
  {
    question: "How do I get information about your API / Programmatic access?",
    answer: <>
      The Antibody Registry does not provide direct API access to the database.
      We utilize the work of our colleagues at the FDI Lab to provide this access to an index of the antibody data.
      Please go to <a href="https://scicrunch.org">https://scicrunch.org</a> &gt; MyAccount &gt; API to gain access.
    </>
  },
  {
  ...
  },
  {
    ...
    answer: <>RRIDs are freely available for reuse under the CC-0 License, including several metadata fields (vendor name, catalog number, proper citation) are available to reuse at any time for any purpose.<br />
      Metadata granted to the Antibody Registry from the antibody companies that enables better search is freely available to search and use, but with important limitations for commercial reuse, as it is covered under the CC-NC license.
      Please contact abr-help -at- scicrunch -dot- org to discuss.
    </>
  },
  {
  ...
  },
  {
    ...
    answer: <>
      We are proud to support the Research Resource Identification, RRID, Initiative as the antibody authority.
      Every antibody listed in the Antibody Registry is also listed in the <a href="https://scicrunch.org/resources">RRID portal</a>.
      Every antibody is also accessible via several resolving services including the RRID resolver (<a href="https://scicrunch.org/resolver/RRID:AB_90755">example</a>).
    </>
  },
  {
